 ...
 * mark, glass reflections need the HDRI, and the full logo PNG (now provided by
 * the user) backs the favicon/loading spinner — these must exist.
 */
const requiredNow = [
...
/**
 * Pending swap-ins (spec §4/§15): the CC0 ambient clips are sourced/swapped in
 * later. Phase 1 degrades gracefully without them (AmbientScheduler swallows
 * missing-clip errors). Tracked here so the gap is visible; flip to
 * `requiredNow` once the files land.
 */
const pending = [
