...
// Destinations fill in over Phases 5–6. Locked rows still render (dimmed) so a
// lower tier can see there is more above them.
const NAV: NavItem[] = [
  { label: 'Orientation', desc: 'Your first days at Redwood Peak.', tier: 'recruit' },
  { label: 'Notices', desc: 'Company-wide bulletins and reminders.', tier: 'recruit' },
  { label: 'Personnel', desc: 'The org chart, and who reports to whom.', tier: 'employee' },
  { label: 'Operations Log', desc: 'Nightly runs and shipment manifests.', tier: 'employee' },
  { label: 'Assignments', desc: 'Your current tasks and their status.', tier: 'employee' },
  { label: 'Witness Dossiers', desc: 'Persons of ongoing interest.', tier: 'high-command' },
  { label: 'Command', desc: 'Directives, and the deep ledger.', tier: 'high-command' },
];
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {unlocked ? (
                    <span className="text-xs text-rw-bone/30">soon</span>
                  ) : (
                    <span className="text-xs text-rw-bone/40" title={`Clearance: ${TIER_LABEL[item.tier]}`}>
                      🔒
                    </span>
                  )}
                </div>
